# 开发构建脚本 - 编译 debug 版本并复制到 demo/addons/gdblive

Write-Host "开始构建 GDBLive (Debug)..." -ForegroundColor Green

# 执行 cargo build (debug)
cargo build
...
if ($LASTEXITCODE -eq 0) {
    ...
    # 复制 Windows DLL
    $sourceDll = "target\debug\GDBLive.dll"
    if (Test-Path $sourceDll) {
        Copy-Item $sourceDll -Destination "$targetDir\GDBLive.windows.x86_64.dll" -Force
        Write-Host "已复制: GDBLive.windows.x86_64.dll (Debug)" -ForegroundColor Cyan
    }
