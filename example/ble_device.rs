...
impl BleDevice {
    ...
    fn connect_async(&mut self) {
        ...
        runtime.spawn(async move {
            ble_debug!("Connection task started for device: {}", address);
            match peripheral.connect().await {
                ...
                Err(e) => {
                    let error = BleError::ConnectionFailed(e.to_string());
                    error.log_error();
    ...
    fn discover_services(&mut self) {
        ...
        runtime.spawn(async move {
            ble_debug!("Service discovery task started for device: {}", address);
            match peripheral.discover_services().await {
                ...
                Err(e) => {
                    let error = BleError::ServiceDiscoveryFailed(e.to_string());
                    error.log_error();
        ...
        runtime.spawn(async move {
            ...
            match characteristic {
                Some(char) => {
                    ...
                    let write_result = if with_response {
                        peripheral
...
impl BleDevice {
    ...
    pub fn new(peripheral: Peripheral, runtime: Arc<Runtime>) -> Gd<Self> {
        ...
        let (address, name) = match properties {
            Ok(Some(props)) => {
                let addr = props.address.to_string();
                let device_name = props.local_name.unwrap_or_default();
                (addr, device_name)
            }
            Ok(None) => (peripheral.address().to_string(), String::new()),
            Err(e) => {
                godot_warn!("Failed to get device properties: {}", e);
                (peripheral.address().to_string(), String::new())
            }
        };
